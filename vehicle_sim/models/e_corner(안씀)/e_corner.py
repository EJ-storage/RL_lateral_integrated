...
class ECornerState:
    """E-Corner 상태 변수 (한 바퀴)"""
    # 출력
    F_s: float = None           # 서스펜션 힘 [N]

    # 중간 상태
    F_x_tire: float = None     # 종방향 타이어 힘 [N]
    F_y_tire: float = None     # 횡방향 타이어 힘 [N]
    F_z: float = None           # 타이어 수직력 [N]
    steering_angle: float = None # 조향 각도 [rad]
    omega_wheel: float = None   # 휠 각속도 [rad/s]
