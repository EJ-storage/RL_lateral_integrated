...
##################Stbw Variable##################
@dataclass
class StbwBrakeParameters:
    tau: float = 0.1
    A_eff: float = 1.2e-3
    V_d: float = 1.0e-7
    p_max_bar: float = 200.0

@dataclass
class StbwBrakeState:
    clamp_force: float = 0.0
##################################################
...
class StbwBrakeModel:
    ...
    def __init__(self, config_path: Optional[str]=None):
        brake_param = load_param('brake', config_path)
        self.params = StbwBrakeParameters(
            tau = float(brake_param.get('tau', 0.1)),
            A_eff = float(brake_param.get('A_eff', 1.2e-3)),
            V_d = float(brake_param.get('V_d', 1.0e-7)),
            p_max_bar = float(brake_param.get('p_max_bar', 200.0))
